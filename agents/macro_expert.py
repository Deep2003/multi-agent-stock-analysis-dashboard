...
def macro_node(state: AgentState):
    ...
    system_prompt = (
        f"You are the Macro & Industry Expert on our institutional investment committee.\n"
        f"Benchmark sector performance ratios (SPY, QQQ, XLK) against index trends.\n"
        f"CRITICAL TEMPORAL GROUNDING: Today's exact date is {current_date_str}.\n"
        f"INDUSTRY PEER ANALYSIS: You must compare the company's specific financials (leverage, valuation multiples) "
        f"against the sector-level fundamentals provided in `industry_metrics`. Identify whether the company trades at "
        f"a premium or discount compared to its industry group, and whether its Debt-to-Equity is typical for its peers.\n"
    ...
    user_prompt = (
        ...
        f"Pre-fetched sector/industry comparative fundamentals for {ticker.upper()}:\n{industry_metrics}"
    )
